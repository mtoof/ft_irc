...
void Command::handlePrivmsg(const Message &msg)
{
    ...
    if (parameters.empty())
	{
        server_ptr_->send_response(client_fd, ERR_NORECIPIENT(server_ptr_->getServerHostname(), client_ptr->getNickname(), "PRIVMSG"));
        return;
    }
	// Check if the recipient exists and is a user or a channel
    if (parameters.size() < 1)
	{
        server_ptr_->send_response(client_fd, ERR_NEEDMOREPARAMS(client_ptr->getClientPrefix(), "PRIVMSG"));
        return;
    }
	...
	{
		server_ptr_->send_response(client_fd, ERR_NOTEXTTOSEND(server_ptr_->getServerHostname(), client_ptr->getClientPrefix()));	// No text to send
		return;
	}
    ...
    if (recipient_ptr)
	{
		if (recipient_ptr->isAway()) // if the recipient is away from the server
            server_ptr_->send_response(client_fd, RPL_AWAY(server_ptr_->getServerHostname(), client_ptr->getNickname(), recipient_ptr->getNickname(), recipient_ptr->getAwayMessage()));
    ...
    if (channel_ptr)	// if the recipient is a channel
	{
		...
		{
            server_ptr_->send_response(client_fd, ERR_CANNOTSENDTOCHAN(server_ptr_->getServerHostname(), client_ptr->getNickname(), recipient));
            return;
        }
    ...
    server_ptr_->send_response(client_fd, ERR_NOSUCHNICK(server_ptr_->getServerHostname(), client_ptr->getNickname(), recipient));  // if the recipient does not exist in the server or the channel does not exist
}
