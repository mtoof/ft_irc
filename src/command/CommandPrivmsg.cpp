...
void Command::handlePrivmsg(const Message &msg)
{
    ...
    if (parameters.empty())
	{
        server_ptr_->sendResponse(client_fd, ERR_NORECIPIENT(server_ptr_->getServerHostname(), lock_client_ptr->getNickname(), "PRIVMSG"));
        return;
    }
	// Check if the recipient exists and is a user or a channel
    if (parameters.size() < 1)
	{
        server_ptr_->sendResponse(client_fd, ERR_NEEDMOREPARAMS(lock_client_ptr->getClientPrefix(), "PRIVMSG"));
        return;
    }
	...
	{
		server_ptr_->sendResponse(client_fd, ERR_NOTEXTTOSEND(server_ptr_->getServerHostname(), lock_client_ptr->getClientPrefix()));	// No text to send
		return;
	}
    ...
    if (recipient_ptr)
	{
		if (recipient_ptr->isAway()) // if the recipient is away from the server
            server_ptr_->sendResponse(client_fd, RPL_AWAY(server_ptr_->getServerHostname(), lock_client_ptr->getNickname(), recipient_ptr->getNickname(), recipient_ptr->getAwayMessage()));
        server_ptr_->sendResponse(recipient_ptr->getFd(), RPL_PRIVMSG(lock_client_ptr->getClientPrefix(), recipient_ptr->getNickname(), message_body)); // send the message to the recipient
        return;
    }
	...
	{
        if (channel_ptr->getModeN() && !channel_ptr->isUserOnChannel(lock_client_ptr->getNickname()))
		{
            server_ptr_->sendResponse(client_fd, ERR_CANNOTSENDTOCHAN(server_ptr_->getServerHostname(), lock_client_ptr->getNickname(), recipient));
            return;
        }
        channel_ptr->broadcastMessage(lock_client_ptr, RPL_PRIVMSG(lock_client_ptr->getClientPrefix(), channel_ptr->getName(), message_body), server_ptr_); // broadcast the message to all users in the channel except the sender
        return;
    }

    server_ptr_->sendResponse(client_fd, ERR_NOSUCHNICK(server_ptr_->getServerHostname(), lock_client_ptr->getNickname(), recipient));  // if the recipient does not exist in the server or the channel does not exist
}
