...
void Command::send_responses_based_on_client_info(const std::shared_ptr<Client> &whois_client_ptr, int fd)
{
	...
	{
		if (channel.second->isUserOnChannel(whois_client_ptr->getNickname()))
			server_->send_response(fd, RPL_WHOISCHANNELS(whois_client_ptr->getNickname(), channel.first));

		// Check if the user is an IRC operator
		if (channel.second->isOperator(whois_client_ptr))
			server_->send_response(fd, RPL_WHOISOPERATOR(
										   whois_client_ptr->getNickname()));

		// Optionally, send idle time and sign-on time if available
		// This data might not be tracked; implement these if your server supports it
		// server_->send_response(fd, RPL_WHOISIDLE(
		//     whois_client_ptr->getNickname(),
		//     whois_client_ptr->getIdleTime(),
		//     whois_client_ptr->getSignOnTime()
		// ));
