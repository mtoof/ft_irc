...
void Command::handleKick(const Message &msg)
{
	...
	{
		server_ptr_->send_response(client_fd, ERR_NEEDMOREPARAMS(client_ptr->getClientPrefix(), "KICK"));
		return;
	}
	...
	for (size_t i = 0; i < targets.size(); i++)
	{
		...
		{
			server_ptr_->send_response(client_fd, ERR_BADCHANMASK(server_ptr_->getServerHostname(), channel_name));
			continue;
		}
		...
		{
			server_ptr_->send_response(client_fd, ERR_USERNOTINCHANNEL(client_ptr->getClientPrefix(), client_ptr->getNickname(), target_nickname, channel_name));
			continue;
		}

		server_ptr_->send_response(client_fd, RPL_KICK(client_ptr->getClientPrefix(), channel_name, target_ptr->getNickname(), reason));
