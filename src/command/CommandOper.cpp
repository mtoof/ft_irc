...
void Command::handleOper(const Message &msg)
{
	...
	std::string requested_nick = lock_client_ptr->getNickname();
	std::string client_ip = lock_client_ptr->getIpAddress();
	std::string client_host = lock_client_ptr->getHostname();
	...
	if (params.size() != 2)
	{
		server_ptr_->sendResponse(client_fd, ERR_NEEDMOREPARAMS(lock_client_ptr->getClientPrefix(), msg.getCommand()));
		return;
	}
			...
			if (it->nick == user_input_nick)
			{
				if (it->password == user_input_pass)
				{
					if (it->hostmask == client_host || it->hostmask == client_ip)
					{
						...
						lock_client_ptr->setModeLocalOp(true);
						// server_ptr_->insertInOperators(std::pair(client_fd, lock_client_ptr)); // Because the mode is not ready yet
						return;
					}
					else
					{
