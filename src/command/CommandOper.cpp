...
void Command::handleOper(const Message &msg)
{
	...
	if (params.size() != 2)
	{
		server_ptr_->send_response(fd, ERR_NEEDMOREPARAMS(client_ptr->getClientPrefix(), msg.getCommand()));
		return;
	}
	...
	{
		server_ptr_->send_response(fd, ERR_NOOPERHOST(server_ptr_->getServerHostname(), requested_nick));
		return;
	}
	else
	{
		...
		for (it = operator_file.begin(); it != operator_file.end();it++)
		{
			if (it->nick == user_input_nick)
			{
				if (it->password == user_input_pass)
				{
					...
					{
						server_ptr_->send_response(fd, RPL_YOUREOPER(server_ptr_->getServerHostname(), requested_nick));
						//Needs to set the mode O for the user as a local operator
						client_ptr->setModeLocalOp(true);
						server_ptr_->insertInOperators(std::pair(fd, client_ptr)); // Because the mode is not ready yet
						return;
					}
					else
					{
				...
				{
					server_ptr_->send_response(fd, ERR_INCORPASS(server_ptr_->getServerHostname(), requested_nick));
					return;
				}
			}
		}
